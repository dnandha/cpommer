...
from utility cimport byte, board_t, Position
...
#            board[bomb_pos]=constants.Item.Passage
...
cdef _all_bomb_real_life(const board_t board, bomb_life, bomb_blast_st, int sz):
    ...
    while True:
        ...
        for i in range(sz):
            for j in range(sz):
                if utility.position_is_wall(board, Position(i,j)) or utility.position_is_powerup(board, Position(i,j)) \
                    or utility.position_is_fog(board, Position(i,j)):
                    continue
                if bomb_life[i,j] < 0+EPSILON:
                    continue
                real_life=get_bomb_real_life(Position(i,j), bomb_real_life_map)
...
def _manhattan_distance(Position pos1, Position pos2):
    #the manhattan distance here is for the specific case
    assert(pos1.row==pos2.row or pos1.col==pos2.col)
    return abs(pos1.row-pos2.row) + abs(pos1.col-pos2.col)

cdef _stop_condition(board_t board, Position pos):
    exclude_agent = True
...
cdef _position_covered_by_bomb(dict obs, board_t board, Position pos, bomb_real_life_map):
    ...
    for d in dirs:
        ...
        while True:
            ...
            if _stop_condition(board, next_pos):
...
cdef _compute_min_evade_step(dict obs, board_t board, list parent_pos_list, Position pos, bomb_real_life):
    flag_cover, min_cover_value, max_cover_value=_position_covered_by_bomb(obs, board, pos, bomb_real_life)
    if not flag_cover:
        return 0
        ...
        for d in dirs:
            ...
            if next_pos in parent_pos_list:
                continue
            x=_compute_min_evade_step(obs, board, parent_pos_list+[next_pos], next_pos, bomb_real_life)
...
cdef _compute_safe_actions(dict obs, board_t board):
    cdef byte x, y
    cdef board_t board2
    ...
    dirs=_all_directions(exclude_stop=True)
    ret=set()
    my_position, board_array, blast_st, bomb_life, can_kick=obs['position'], obs['board'], obs['bomb_blast_strength'], obs['bomb_life'], obs['can_kick']

    kick_dir=None
    bomb_real_life_map=_all_bomb_real_life(board, bomb_life, blast_st, board_array.shape[0])
    flag_cover_passages=[]
    for direction in dirs:
        position = utility.get_next_position(Position(my_position[0], my_position[1]), direction)
        ...
        #    obs['board'][position]=constants.Item.Passage
        #    gone_flame_pos=position

        if utility.position_is_passage(board, position) or utility.position_is_powerup(board, position):
            my_id=board[my_position[0]][my_position[1]]
            board[my_position[0]][my_position[1]] = constants.Item.Bomb if bomb_life[my_position]>0  else constants.Item.Passage
            flag_cover, min_cover_value , max_cover_value=_position_covered_by_bomb(obs, board, position, bomb_real_life_map)
            flag_cover_passages.append(flag_cover)
            if not flag_cover:
                ret.add(direction)
            else:
                min_escape_step=_compute_min_evade_step(obs, board, [position], position, bomb_real_life_map)
                assert(min_escape_step>0)
                if min_escape_step  < min_cover_value:
                    ret.add(direction)
            board[my_position[0]][my_position[1]] = my_id
        if gone_flame_pos is not None:
            board[gone_flame_pos[0]][gone_flame_pos[1]] = constants.Item.Flames
    ...
    flag_cover, min_cover_value, max_cover_value=_position_covered_by_bomb(obs, board, my_position, bomb_real_life_map)
    if flag_cover:
        min_escape_step=_compute_min_evade_step(obs, board, [None, my_position], my_position, bomb_real_life_map)
        if min_escape_step < min_cover_value:
            ret.add(constants.Action.Stop)
    else:
        ret.add(constants.Action.Stop)
    ...
    if not (obs['ammo'] <=0 or obs['bomb_life'][obs['position']]>0):
        ...
        if BOMBING_TEST == 'simple':
            if not flag_cover:
                ret.add(constants.Action.LayBomb)
        elif BOMBING_TEST == 'simple_adjacent':
            if (not flag_cover) and (not any(flag_cover_passages)): 
                ret.add(constants.Action.LayBomb)
        else: #lookahead
            if (constants.Action.Stop in ret) and len(ret)>1 and (kick_dir is None):
                obs2=copy.deepcopy(obs)
                board_array = obs2['board']
                for x in range(board_array.shape[1]):
                    for y in range(board_array.shape[0]):
                        board2[x][y] = board_array[x][y]
                my_pos=obs2['position']
                obs2['board'][my_pos]=constants.Item.Bomb
                obs2['bomb_life'][my_pos]=min_cover_value if flag_cover else 10
                obs2['bomb_blast_strength'][my_pos]=obs2['blast_strength']
                bomb_life2, bomb_blast_st2 = obs2['bomb_life'], obs2['bomb_blast_strength']
                bomb_real_life_map=_all_bomb_real_life(board2, bomb_life2, bomb_blast_st2, obs2['board'].shape[0])
                min_evade_step=_compute_min_evade_step(obs2, board2, [None, my_position], my_pos, bomb_real_life_map)
...
cdef list get_filtered_actions(dict obs):
    ...
    ret=_compute_safe_actions(obs, board)
    if len(ret)!=0:
        return list(ret)
    else:
        return [constants.Action.Stop]

cdef _kick_test(board_t board, blast_st, bomb_life, Position my_position, direction):
